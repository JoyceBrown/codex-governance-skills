...
When the user-level Codex hooks are enabled, `SessionStart` validates and restores an existing project ledger. `UserPromptSubmit` creates a private turn guard for likely requirement changes without retaining prompt text, and write-capable `PreToolUse` calls are denied until the requirement revision/hash advances. `PreCompact`, `PostCompact`, and `SessionStart(source=compact)` compare task ID, checkpoint, revision, and requirements hash across compaction. `Stop` requests one continuation when state still needs recovery or checkpointing. Hook failures degrade to a bounded warning; they must not bypass Codex hook trust or block unrelated sessions.
