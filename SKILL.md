...
When the user-level Codex hooks are enabled, `SessionStart` validates and restores an existing project ledger. `UserPromptSubmit` records only privacy-safe telemetry for a lexical requirement hint; it does not create a write gate because a hint is not proof of a requirement change. Write-capable `PreToolUse` calls are denied only for verified ledger corruption or current-state drift. `PreCompact`, `PostCompact`, and `SessionStart(source=compact)` compare task ID, checkpoint, revision, and requirements hash across compaction. `Stop` reports ledger bookkeeping inconsistencies as an advisory without requesting continuation; only an active recovery condition may request one bounded continuation. Hook failures degrade to a bounded warning; they must not bypass Codex hook trust or block unrelated sessions.
